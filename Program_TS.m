...
P_max=2; P_min=0;
Q_max=20;  Q_min=0;
P_ref_gap=(P_max-P_min)/(Num_Approximate-1);
...
for i=1:size(UnitThermalGenerators,1)-1
    ...
    P_min=UnitThermalGenerators{i+1,5};
    marign_cost=UnitThermalGenerators{i+1,12};
    
    AllThermalGenerators{i}=struct('location',location,'capacity',capacity,'P_min',P_min,'marign_cost',marign_cost);
end
%风力发电机组数据处理
...
for i=1:size(WindFarmGenerators,1)-1
    ...
    P_min=WindFarmGenerators{i+1,5};
    ...
    AllWindFarmGenerators{i}=struct('location',location,'capacity',capacity,'P_min',P_min,'generationcost',generationcost,'typicaloutput',typicaloutput);
end
%光伏发电机组数据处理
...
for i=1:size(PhotovoltaicGenerators,1)-1
    ...
    P_min=PhotovoltaicGenerators{i+1,5};
    ...
    AllPhotovoltaicGenerators{i}=struct('location',location,'capacity',capacity,'P_min',P_min,'generationcost',generationcost,'typicaloutput',typicaloutput);
end
%将所有机组数据整合至一个元胞之中
...
for i=1:Num_Gen                                        %KKT条件转换后约束1
    for j=1:Seg_offer
        for k=1:W
            for m=1:T
                if ismember(i,RE_Gens)
                    Const_RT=[Const_RT,Offer_Price(RE_Gens==i,j)-LMP_RT(AllGens{i}.location,k,m)+u_RT_Gmax(i,j,k,m)-u_RT_Gmin(i,j,k,m)+u_RT_GTmax(i,k,m)-u_RT_GTmin(i,k,m)==0];
                else
                    Const_RT=[Const_RT,Marign_Cost(i,j)-LMP_RT(AllGens{i}.location,k,m)+u_RT_Gmax(i,j,k,m)-u_RT_Gmin(i,j,k,m)+u_RT_GTmax(i,k,m)-u_RT_GTmin(i,k,m)==0];
                end
            end
        end
    end
...
for i=1:Num_Load                                       %KKT条件转换后约束2
    for j=1:Seg_bid
        for k=1:W
            for m=1:T
                Const_RT=[Const_RT,LMP_RT(AllLoads{i}.location,k,m)-Bid_Price(i,j)+u_RT_Dmax(i,j,k,m)-u_RT_Dmin(i,j,k,m)==0];
            end
        end
    end
end
...
for i=1:Num_Node                                       %KKT条件转换后约束3
    for j=1:W
        for k=1:T
            New_Const=0;
            for m=1:size(AllNodes{i}.connectnodes,1)
                New_Const=New_Const+B(i,AllNodes{i}.connectnodes(m))*(LMP_RT(i,j,k)-LMP_RT(AllNodes{i}.connectnodes(m),j,k))+B(i,AllNodes{i}.connectnodes(m))*(v_RT_Lmax(i,AllNodes{i}.connectnodes(m),j,k)-v_RT_Lmax(AllNodes{i}.connectnodes(m),i,j,k));
            end
            if i~=1
                Const_RT=[Const_RT,New_Const+e_RT_max(i,j,k)-e_RT_min(i,j,k)==0];
            else
                Const_RT=[Const_RT,New_Const+e_RT_max(i,j,k)-e_RT_min(i,j,k)+e_RT1(j,k)==0];
            end
        end
    end
end

for i=1:Num_Gen                                       %实时市场发电机每一时段每一报价段每一场景出力互补松弛约束
    for j=1:Seg_offer
        for k=1:W
            for m=1:T
                Const_RT=[Const_RT,0<=(P_RT_G(i,j,k,m)-P_RT_Gmin(i,j,k,m))<=M*c_RT_Gmin(i,j,k,m)];
                Const_RT=[Const_RT,0<=u_RT_Gmin(i,j,k,m)<=M*(1-c_RT_Gmin(i,j,k,m))];
                Const_RT=[Const_RT,0<=(P_RT_Gmax(i,j,k,m)-P_RT_G(i,j,k,m))<=M*c_RT_Gmax(i,j,k,m)];
                Const_RT=[Const_RT,0<=u_RT_Gmax(i,j,k,m)<=M*(1-c_RT_Gmax(i,j,k,m))];
            end
        end
    end
end

for i=1:Num_Gen                                       %实时市场发电机每一时段每一场景出力互补松弛约束
    for j=1:W
        for k=1:T
            Const_RT=[Const_RT,0<=(sum(P_RT_G(i,:,j,k))-P_RT_GTmin(i,j,k))<=M*c_RT_GTmin(i,j,k)];
            Const_RT=[Const_RT,0<=u_RT_GTmin(i,j,k)<=M*(1-c_RT_GTmin(i,j,k))];
            Const_RT=[Const_RT,0<=(P_RT_GTmax(i,j,k)-sum(P_RT_G(i,:,j,k)))<=M*c_RT_GTmax(i,j,k)];
            Const_RT=[Const_RT,0<=u_RT_GTmax(i,j,k)<=M*(1-c_RT_GTmax(i,j,k))];           
        end
    end
end

for i=1:Num_Load                                      %实时市场用户每一时段每一用电段每一场景用电量互补松弛约束
    for j=1:Seg_bid
        for k=1:W
            for m=1:T
                Const_RT=[Const_RT,0<=(P_RT_D(i,j,k,m)-P_Dmin(i,j,m))<=M*c_RT_Dmin(i,j,k,m)];
                Const_RT=[Const_RT,0<=u_RT_Dmin(i,j,k,m)<=M*(1-c_RT_Dmin(i,j,k,m))];
                Const_RT=[Const_RT,0<=(P_Dmax(i,j,m)-P_RT_D(i,j,k,m))<=M*c_RT_Dmax(i,j,k,m)];
                Const_RT=[Const_RT,0<=u_RT_Dmax(i,j,k,m)<=M*(1-c_RT_Dmax(i,j,k,m))];
            end
        end
    end
end
...
for i=1:Num_Node                                      %实时市场线路流过潮流互补松弛约束
    for j=1:W
        for k=1:T
            for m=1:size(AllNodes{i}.connectnodes,1)
                Const_RT=[Const_RT,0<=(P_Lmax(i,AllNodes{i}.connectnodes(m))-B(i,AllNodes{i}.connectnodes(m))*(Delta_RT(i,j,k)-Delta_RT(AllNodes{i}.connectnodes(m),j,k)))<=M*c_RT_Lmax(i,AllNodes{i}.connectnodes(m),j,k)];
                Const_RT=[Const_RT,0<=v_RT_Lmax(i,AllNodes{i}.connectnodes(m),j,k)<=M*(1-c_RT_Lmax(i,AllNodes{i}.connectnodes(m),j,k))];
            end
        end
    end
end
...
for i=1:Num_Node                                      %实时市场电压角互补松弛约束
    for j=1:W
        for k=1:T
            Const_RT=[Const_RT,0<=(Delta_RT(i,j,k)+pi)<=M*c_RT_Jmin(i,j,k)];
            Const_RT=[Const_RT,0<=e_RT_min(i,j,k)<=M*(1-c_RT_Jmin(i,j,k))];
            Const_RT=[Const_RT,0<=(pi-Delta_RT(i,j,k))<=M*c_RT_Jmax(i,j,k)];
            Const_RT=[Const_RT,0<=e_RT_max(i,j,k)<=M*(1-c_RT_Jmax(i,j,k))];
        end
    end
end
...
for i=1:Num_Node                                      %实时市场功率平衡约束
    for j=1:W
        for k=1:T
            New_Const=0;
            for m=1:Num_Gen
                if AllGens{m}.location==i
                    New_Const=New_Const+sum(P_RT_G(m,:,j,k));
                end  
            end
            for m=1:Num_Load
                if AllLoads{m}.location==i
                    New_Const=New_Const-sum(P_RT_D(m,:,j,k));
                end
            end
            for m=1:size(AllNodes{i}.connectnodes,1)
                New_Const=New_Const-B(i,AllNodes{i}.connectnodes(m))*(Delta_RT(i,j,k)-Delta_RT(AllNodes{i}.connectnodes(m),j,k));
            end
...
for i=1:W                                            %实时市场参考点电压角约束
    for j=1:T
        Const_RT=[Const_RT,Delta_RT(1,i,j)==0];
    end
end
disp('底层问题二问题约束添加完毕');
%%
    ...
    for j=1:T
        ...
        for k=1:size(RE_Gens,1)
            for m=1:Num_Approximate
                New_Const=New_Const-(P_min+(m-1)*P_ref_gap)*RT_Ancillary1(k,i,j,m);
            end
        end
...
for i=1:size(RE_Gens,1)
    for j=1:T
        ...
        for k=1:Num_Approximate
            New_Const=New_Const+(P_min+(k-1)*P_ref_gap)*RT_Ancillary2(i,j,k);
        end
...
Obj=-(R_DA+R_RT+R_TGC);
%开始求解
Ops=sdpsettings('verbose',1,'solver','gurobi');    %设置求解参数
%Const=[Const_Upper,Const_DA,Const_RT,Const_Liner_DA,Const_Liner_RT,Const_Liner_TGC];
disp('开始求解');

Result=solvesdp([Const_Upper,Const_DA,Const_Liner_DA],-R_DA,Ops);
if Result.problem==0 
